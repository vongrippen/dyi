...
#        @chart_options[:filter] = "url(##{Drawing::Filter::DropShadow.new(@canvas, dropshadow_blur_std, dropshadow_dx, dropshadow_dy).id})" if show_dropshadow?
      ...
      def draw_bar(id, color, settings)
        ...
        zero = value_position_on_chart(margin_top, settings, 0, true)
        values.each_with_index do |value, i|
          ...
          mt = margin_top
          if value < 0
            brush.draw_rectangle(bar_group, [x, zero], bar_width, y - zero)
          else
            brush.draw_rectangle(bar_group, [x, y], bar_width, zero - y)
          end
          if show_data_label?
